...
// Vérification reCAPTCHA v2 invisible (appel serveur → Google)
const verifyRecaptchaV2 = async ({ token, ip }) => {
  if (!isRecaptchaV2Active()) {
    return { ok: true, skipped: true, reason: 'inactive' }
  }

  const { recaptchaV2SecretKey: secret } = getServerEnv()
  ...
  try {
    ...
    if (!data.success) {
      console.warn('[Anti-Spam] Échec API reCAPTCHA v2', data)
      return { ok: false, reason: 'api_failure', data }
    }

    return { ok: true }
  } catch (error) {
    console.error('[Anti-Spam] Erreur reCAPTCHA v2:', error)
    if (failOnError) {
      return { ok: false, reason: 'exception' }
    }
    return { ok: true, skipped: true, reason: 'exception' }
...
export const antiSpamMiddleware = async (req, options = {}) => {
  ...
  // 5. reCAPTCHA (v2 pour inscriptions, v3 pour contact/quiz)
  if (recaptcha && recaptcha.enabled) {
    const isV2 = recaptcha.version === 'v2'
    const shouldVerify = isV2 ? isRecaptchaV2Active() : isRecaptchaActive()

    if (shouldVerify) {
      const recaptchaResult = isV2
        ? await verifyRecaptchaV2({ token: recaptchaToken, ip })
      ...
      if (!recaptchaResult.ok) {
        console.log(
          `[Anti-Spam] reCAPTCHA${isV2 ? ' v2' : ''} rejeté - IP: ${ip}, raison: ${recaptchaResult.reason || 'unknown'}`
        )
        return {
          success: false,
          error: 'validation_failed',
