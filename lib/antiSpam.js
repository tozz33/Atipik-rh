...
const verifyRecaptcha = async ({ token, ip, action, minScore }) => {
  ...
  // Si la config est absente, on ne bloque pas la requête, on log seulement
  if (!secret || !siteKey) {
    console.warn('[Anti-Spam] reCAPTCHA non configuré (clés manquantes), vérification ignorée')
  ...
  try {
    ...
    // En cas d’erreur réseau ou autre, on ne bloque pas la requête
...
const checkAkismet = async ({ ip, userAgent, referrer, formData, type }) => {
  ...
  if (!apiKey || !siteUrl) {
    console.warn('[Anti-Spam] Akismet non configuré (clé ou URL manquante), vérification ignorée')
    return { ok: true, skipped: true, reason: 'missing_config' }
  }
  ...
  try {
    ...
    // En cas d’erreur réseau ou configuration, on ne bloque pas la requête
    return { ok: true, skipped: true, reason: 'exception' }
  }
}
...
export const antiSpamMiddleware = async (req, options = {}) => {
  ...
  // 5. reCAPTCHA v3 (optionnel, par endpoint)
  ...
  // 6. Akismet (optionnel, par endpoint)
  if (akismet && akismet.enabled) {
